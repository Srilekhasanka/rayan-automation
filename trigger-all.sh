#!/bin/bash
# ─── Trigger tests on all 20 EC2 instances simultaneously ────────────────────
#
# Usage:
...
INDEX=1
for IP in $IPS; do
  echo "  [$INDEX/$COUNT] $IP — starting tests in background..."
  ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no -o ConnectTimeout=10 \
    "ubuntu@${IP}" \
    "nohup sudo -u appuser bash -c 'export DISPLAY=:99 && cd /home/appuser/app && npm test' > /home/appuser/test-output.log 2>&1 &" &
  INDEX=$((INDEX + 1))
done

wait
echo ""
echo "Tests triggered on all $COUNT instances."
echo ""
